...
MODULE_EXECUTE(modeling)
{
  ...
  self->state->p += Q;
  DEBUG_PRINT("M: Predicted state: x = %d, p = %d\n", self->state->x, self->state->p);
  // Kalman update
  ...
  if (detS == 0) {
    DEBUG_PRINT("M: S is not invertible!\n");
    return;
  }
  DEBUG_PRINT("M: det(S) = %d\n", detS);
  ...
  DEBUG_PRINT("M: Estimated state: x = %d, p = %d\n", self->state->x, self->state->p);
}
