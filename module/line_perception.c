...
MODULE_EXECUTE(line_perception)
{
  if (self->request->type == REQUEST_INIT) {
    ...
    if (self->calibration_mask == 0x0f && self->calibration_timer > CALIBRATION_TIMEOUT) {
      DEBUG_PRINT("LP: Done with calibration.\n");
